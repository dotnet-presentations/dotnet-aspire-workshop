...
# 대시보드 및 Aspire App Host 프로젝트를 이용한 오케스트레이션

Aspire는 분산 애플리케이션 내의 리소스와 종속성을 표현하기 위한 API를 제공합니다. 이러한 API 외에도 몇 가지 흥미로운 시나리오를 가능하게 하는 도구를 제공합니다. 오케스트레이터는 로컬 개발 목적으로 사용합니다.

계속하기 전에 Aspire에서 사용하는 일반적인 용어를 확인해보세요:
...
1. `AppHost`라는 새 프로젝트를 솔루션에 추가합니다:
    ...
    - `Aspire App Host` 프로젝트 템플릿을 선택합니다.
...
1. Aspire 대시보드가 기본 브라우저에서 열리고 애플리케이션의 리소스 및 종속성이 표시됩니다.

    ![Aspire 대시보드](.../media/dashboard.png)
...
1. `Traces` 탭을 선택하고 API를 호출하는 트레이스에서 `View`를 선택합니다.
    ...
    ![Aspire 대시보드](.../media/dashboard-trace.png)]

1. `Metrics` 탭을 탐색하여 `Api` 및 `MyWeatherHub` 프로젝트의 메트릭을 확인합니다.

    ![Aspire 대시보드](.../media/dashboard-metrics.png)
...
1. 오류를 생성한 후 대시보드의 `Structured` 탭이 자동으로 업데이트가 되며 오류를 표시합니다.
    ...
    ![Aspire 대시보드](.../media/dashboard-error.png)

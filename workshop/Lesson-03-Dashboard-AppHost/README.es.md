# Panel de control y orquestación con Aspire App Host

Aspire proporciona APIs para expresar recursos y dependencias dentro de tu aplicación distribuida. Además de estas APIs, hay herramientas que permiten algunos escenarios interesantes. El orquestador está destinado a fines de desarrollo local.

Antes de continuar, considera algunos términos comunes utilizados en Aspire:
...
* Selecciona la plantilla de proyecto `Aspire App Host`.
...
1. El Panel de control de Aspire se abrirá en tu navegador predeterminado y mostrará los recursos y dependencias de tu aplicación.

 ![Panel de control de Aspire](.../media/dashboard.png)
...
1. Selecciona la pestaña `Traces` y selecciona `Ver` en un rastro donde se llame a la API.
 ...
 ![Panel de control de Aspire](.../media/dashboard-trace.png)

1. Explora la pestaña `Métricas` para ver las métricas de los proyectos `Api` y `MyWeatherHub`.

 ![Panel de control de Aspire](.../media/dashboard-metrics.png)
...
1. Después de generar los errores, la pestaña `Estructurado` se actualizará automáticamente en el panel de control y observa que se muestran los errores.
 ...
 ![Panel de control de Aspire](.../media/dashboard-error.png)
